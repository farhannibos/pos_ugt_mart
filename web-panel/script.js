...
function renderLangganan() {
    ...
    const rows = _lanData.filter(r => r.nama_toko.toLowerCase().includes(q));

    if (rows.length === 0) {
        tbody.innerHTML = '<tr><td colspan="6" style="text-align:center;padding:32px;color:#94A3B8">Tidak ada data</td></tr>';
    } else {
        tbody.innerHTML = rows.map(r => {
            const isPremium = r.plan === 'premium';
            const exp = r.expired_at ? new Date(r.expired_at) : null;
            const expStr = exp ? `${exp.getDate().toString().padStart(2,'0')}/${(exp.getMonth()+1).toString().padStart(2,'0')}/${exp.getFullYear()}` : '—';
            const now = new Date();
    ...
    const totalPremium = _lanData.filter(r => r.plan === 'premium' && r.expired_at && new Date(r.expired_at) > new Date()).length;
    const totalFree = _lanData.filter(r => r.plan !== 'premium').length;
    const expSoon = _lanData.filter(r => {
        if (!r.expired_at) return false;
        const diff = Math.floor((new Date(r.expired_at) - new Date()) / 86400000);
        return diff >= 0 && diff <= 7;
    }).length;

    const elPremium = document.getElementById('lan-total-premium');
    const elFree = document.getElementById('lan-total-free');
    const elExp = document.getElementById('lan-exp-soon');
    if (elPremium) elPremium.textContent = totalPremium;
    if (elFree) elFree.textContent = totalFree;
    if (elExp) elExp.textContent = expSoon;
...
function populateTokoSelect() {
    const sel = document.getElementById('akt-id-toko');
    if (!sel) return;
    sel.innerHTML = '<option value="">-- Pilih Toko --</option>' +
        _lanData.map(r => `<option value="${r.id}">${r.nama_toko}</option>`).join('');
}

function showModalAktivasiPremium() {
    document.getElementById('akt-id-toko').value = '';
    document.getElementById('akt-bulan').value = '3';
    ...
    document.getElementById('modal-aktivasi').style.display = 'flex';
    lucide.createIcons();
}

function showModalAktivasiPremiumFor(idToko, namaToko) {
    showModalAktivasiPremium();
    document.getElementById('akt-id-toko').value = idToko;
}

function closeModalAktivasi() {
    document.getElementById('modal-aktivasi').style.display = 'none';
}

async function doAktivasiPremium() {
    const idToko = parseInt(document.getElementById('akt-id-toko').value);
    const bulan = parseInt(document.getElementById('akt-bulan').value);
    ...
    if (!idToko) { errEl.textContent = 'Pilih toko terlebih dahulu'; errEl.style.display='block'; return; }
    if (!nominal) { errEl.textContent = 'Isi nominal pembayaran'; errEl.style.display='block'; return; }

    const adminName = JSON.parse(localStorage.getItem('ugt_user') || '{}').nama || 'Admin';
    btn.disabled = true;
    ...
    try {
        const { data, error } = await _sb.rpc('aktivasi_premium', {
            ...
            p_bulan:      bulan,
            p_nominal:    nominal,
            p_keterangan: keterangan || `Aktivasi ${bulan} bulan`,
            p_admin:      adminName,
        });
        if (error) throw error;
